...
```tsx
it('Checks dropdown has options 20, 10, 5, 1 in that order (no 40 or 80)', () => {
  ...
  const options = Array.from(checksSelect.querySelectorAll('option'))
    .map(o => o.value)
    .filter(v => v !== '')
  expect(options).toEqual(['20', '10', '5', '1'])
})
```
...
```ts
const CHECKS_OPTIONS = ['20', '10', '5', '1']
```
...
git commit -m "feat: reorder checks filter [20,10,5,1], remove 40 and 80"
```
...
2. `feat: reorder checks filter [20,10,5,1], remove 40 and 80`
