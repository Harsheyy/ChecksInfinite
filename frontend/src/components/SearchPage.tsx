...
export function SearchPage({ getLikeInfo }: SearchPageProps) {
  ...
  // Client-side trait filter for pool-based paths (ids/wallet); global already filtered server-side
  const visiblePermutations = useMemo(() => {
    if (!isIdsActive && !isWalletActive) return rawPermutations
    if (!hasActiveSearchFilters(filters)) return rawPermutations
    return rawPermutations.filter(p => matchesSearchFilters(p.nodeAbcd.attributes, filters))
    ...
    <div className="searchpage">
      {showEmptyForm ? (
        ...
        <SearchQueryBar
          ...
          onClearTrait={handleClearTrait}
          onEdit={handleEdit}
          totalMatches={visiblePermutations.length}
          capped={isGlobalActive && global.capped}
          globalTotal={isGlobalActive ? global.totalMatches : undefined}
